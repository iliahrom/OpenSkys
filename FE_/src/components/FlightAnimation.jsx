...
const FlightPoints = () => {
  ...
  useEffect(() => {
    ...
    if (curr.startsWith("hover")) {
      ...
      setDronePos(curr);
      const timeout = setTimeout(() => setStep((s) => s + 1), 1000);
      return () => clearTimeout(timeout);
    }
  }, [step, start, end]);
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.grid}>
        <button className={styles.base}>Base</button>
        {[0, 3, 6].map((row) => (
        ...
        <img
          ...
          style={{
            top: `calc(${pointCoordinates[dronePos].row * 120}px + ${
              hovering ? 50 : 0
            }px)`,
            left: `${pointCoordinates[dronePos].col * 120 + 20}px`,
          }}
          alt="drone in flight"
        />
      </div>
