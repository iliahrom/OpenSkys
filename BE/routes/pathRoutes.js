...
router.post('/', async (req, res) => {
  ...
  try {
    const [result] = await db.getConnection().promise().execute(
...
router.get('/', async (req, res) => {
  try {
    const [paths] = await db.getConnection().promise().execute(
      "SELECT * FROM paths"
    );

    const [pathPoints] = await db.getConnection().promise().execute(
      "SELECT * FROM path_points ORDER BY position"
    );
